...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 12-03-2019 a las 00:11:29
...
-- Procedimientos
--
CREATE DEFINER=`infocat_root`@`localhost` PROCEDURE `buscarEmpleado` (IN `campo` VARCHAR(20))  NO SQL
...
CREATE DEFINER=`infocat_root`@`localhost` PROCEDURE `empleados_pormes` (IN `ano` INT, IN `mes` INT, IN `empresa` INT)  NO SQL
...
CREATE DEFINER=`infocat_root`@`localhost` PROCEDURE `existe_usuario` (IN `codBarras` VARCHAR(8))  NO SQL
...
CREATE DEFINER=`infocat_root`@`localhost` PROCEDURE `proce1` (IN `codBarra` VARCHAR(8))  BEGIN
...
CREATE DEFINER=`infocat_root`@`localhost` PROCEDURE `proce2` ()  NO SQL
select * from usuarios where idUsuario=1$$

CREATE DEFINER=`infocat_root`@`localhost` PROCEDURE `registro` (IN `id` INT)  NO SQL
UPDATE `registro_asistencias` 
SET
`regEntrada02`=DATE_FORMAT(NOW(),'%T')
WHERE IdUsuario=id$$

CREATE DEFINER=`infocat_root`@`localhost` PROCEDURE `registro_detallepormes` (IN `ano` INT, IN `mes` INT, IN `empresa` INT, IN `id` INT)  NO SQL
...
CREATE DEFINER=`infocat_root`@`localhost` PROCEDURE `reporte` (IN `mesFecha` INT, IN `anoFecha` INT)  NO SQL
...
CREATE DEFINER=`infocat_root`@`localhost` PROCEDURE `sp_ingresos` ()  NO SQL
begin
...
(2, 'Hector', 'Arteaga', '26509773', 1, 1),
(3, 'Eliseo', 'Villafañe', '25468406', 1, 1),
(4, 'Pilar', 'Mateo', '71207061', 1, 1),
(5, 'Sheyla', 'Quispe', '77496250', 1, 1),
(6, 'Luz', 'Cano', '74168782', 1, 1),
(7, 'Yuri', 'Huaycuch', '77704076', 1, 1),
(8, 'Mayte', 'Miranda', '74233384', 1, 1),
(9, 'Danitza', 'Cruzatt', '74902277', 1, 1);
